...
*(Amended 2026-09-08, review round six; confirmed by the owner.)* One explicit exception: when a submission was **rejected by validation** (422) and the customer confirms again, the client checks the rejected key once more, and a "not found" on that check, recognised as this API's own `not_found` body, permits a new key. Without it a rejected submission could never be re-confirmed, and per-intent coordination is ruled out above. That "not found" does not prove absence of effects (see "The validation window" and "The re-confirmation check" under "Where this still breaks"); a network failure, a 5xx, or any other answer to that check keeps the key and permits nothing.
...
**The validation window.** *(Added 2026-09-07, plan review.)* A new intent is validated against the menu before its row is inserted. Two concurrent requests carrying the same key, with a price change landing between their validations, can diverge: one validates and inserts, the other fails validation and is told "rejected" while an order under that key exists and may be paid. A lookup by key after a failed validation narrows this window but does not close it — the winner may not have committed yet. It requires a concurrent same-key replay *and* a menu change inside the same few milliseconds. No per-intent coordination or lock is added for it; the residual window is recorded here rather than papered over, and the client's rejection screen keeps "Start new order" available so the customer can recover.

**The re-confirmation check.** *(Added 2026-09-08 at the owner's instruction.)* The check the client makes when the customer confirms again after a rejection narrows the window above; it does not close it. A `not_found` can be returned because the concurrent request that accepted the key has not committed yet, so re-confirmation after a rejection remains theoretically able to create a second intent while the first can still be accepted. Same shape as the validation window, same reason for not adding a lock: it needs a concurrent same-key request and a commit landing between the check and the new key. The exception is stated as what it is — the only signal available without coordination — not as a fix.
